...
$id = $_GET['id'];
$result = mysql_query("SELECT * FROM `tbl_complaint` complaint inner join `tbl_student` student on 
            complaint.student_id = student.student_id WHERE complaint.complaint_id='$id'");
...
<div class="div">
        <div class="col-lg-12 ">
          ...
          <table>
          <?php
            $query1=mysql_query("SELECT * FROM `tbl_complaint` complaint inner join `tbl_student` student on 
            complaint.student_id = student.student_id WHERE complaint.complaint_id='$id'");
            while( $arry=mysql_fetch_array($query1) ) {

              $id = $arry['complaint_id'];
              $name = $arry['student_name'];
              $email = $arry['student_email'];
              $phone_no = $arry['student_contact'];
