...
$id = $_GET['id'];
$result = mysql_query("SELECT * FROM `tbl_complaint` complaint inner join `tbl_college` college on 
            complaint.college_id = college.college_id WHERE complaint.complaint_id='$id'");
...
<div class="div">
        <div class="col-lg-12 ">
          ...
          <table>
          <?php
            $query1=mysql_query("SELECT * FROM `tbl_complaint` complaint inner join `tbl_college` college on 
            complaint.college_id = college.college_id WHERE complaint.complaint_id='$id'");
            while( $arry=mysql_fetch_array($query1) ) {

              $id = $arry['complaint_id'];
              $name = $arry['college_name'];
              $email = $arry['college_email'];
              $phone_no = $arry['college_contact'];
